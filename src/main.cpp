...
hittable_list final_scene() {
    ...
    int num_mats = 100;
    for(int c = 0; c < num_mats; ++c) {
        if(c < num_mats / 2) {
            materials.push_back(std::make_shared<lambertian>(color::random(0.1, 1.0)));
        } else {
            materials.push_back(std::make_shared<metal>(color::random(0.7, 0.95), 1.0));
        }
    }
    ...
    int nb = 1000;
    for (int j = 0; j < nb; j++) {
        auto random_point = point3::random(0,200);
        auto size = vec3::random(4, 10);
        auto material = materials[random_int(0, (int)materials.size() - 1)];
        boxes3.add(std::make_shared<box>(random_point, random_point + size, material));
    }
    
    objects.add(
        std::make_shared<translate>(
            std::make_shared<bvh_node>(boxes3, 0.0, 1.0),
            vec3(+200,270,395)
        )
    );
...
color ray_color(ray const & r, const color& background, const hittable& world, int depth) {
    hit_record rec;
